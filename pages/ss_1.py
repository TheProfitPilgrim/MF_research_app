...
if st.button("Select"):
    if selection_mode == "Current":
        ...
        if not is_valid:
            ...
            df_top_rebalance_bt, pf_bt_yes_return, index_return, unique_funds_count, num_rebalances = rebalancing_backtest.backtest_with_rebalancing(start_date, end_date, min_days, top_n_alpha, rebalance_freq)
            
            st.session_state.df_top_rebalance_bt = df_top_rebalance_bt
            st.session_state.pf_bt_yes_return = pf_bt_yes_return
            st.session_state.index_return = index_return
            st.session_state.unique_funds_count = unique_funds_count
...
if "df_top_rebalance_bt" in st.session_state:
    ...
    st.write(f"### Number of Unique Funds Selected: {st.session_state.unique_funds_count}")
    st.write(f"### Number of Rebalances: {st.session_state.num_rebalances}")

    st.write("### Rebalancing PFs")
    st.dataframe(st.session_state.df_top_rebalance_bt)